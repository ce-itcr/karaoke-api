...
describe('Comunicación con la base de datos', function() {
    ...
    it('Buscar canción', function(done){
        chai.request('http://localhost:5000').get('/karaoke/getSong/{"songName":"Fuentes de Ortiz","songAuthor":"Ed Maverick" }')
        .end((err, res) => {
            expect(res).to.have.status(200);
            done();
        });
    ...
    it('Modificar canción', function(done){
        chai.request('http://localhost:5000').put('/karaoke/updateSong/{ "songName":"Fuentes de Ortiz", "songAuthor":"Ed Maverick"}').send({"modificationAuthor":"Momboñombo Moñagallo"})
        .end((err, res) => {
    ...
    it('Borrar canción', function(done){
        chai.request('http://localhost:5000').delete('/karaoke/deleteSong/{"songName":"Fuentes de Ortiz", "songAuthor":"Ed Maverick"}')
        .end((err, res) => {
